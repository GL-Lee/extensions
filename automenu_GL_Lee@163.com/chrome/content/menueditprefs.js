...
var Dialog = {
  ...
  init: function() {
    ...
    document.getElementById("modelist").selectedIndex = parseInt(document.getElementById("modelist").getAttribute("selectedIdx"));
  ...
  moveToHbox: function(){
  	...
  	var panels = window.arguments[0].children;
    var panel;
    var hbox;

    if(panels.length == 1){
    	setTimeout(function() {  MenuEdit.moveToHbox();}, 1000);
    	return ;
    }
    for(var i = 0; i < panels.length; i++){
    	var hbox = document.createElement("hbox");
    	if(panels[i].localName=='panel'){
	      var items = panels[i].children;
	      var m = 0;
	      for(var j = 0, len2 = items.length; j < len2; j++){
	      	if(items[0].localName!='hbox'){
	      		m++;
	      		continue;
	      	}
	      	if(j%4 == 0){
	        ...
	        items[0].setAttribute("tooltiptext", label);
	        items[0].label = "";
	        var words = label.split(" ");
	        for(var l = 0; l < words.length; l++){
	        	items[0].children[0].value+=words[l].substring(0,1);
	        }
	        items[0].children[2].src="chrome://automenu_GL/content/enabled.png";
	      	vbox.appendChild(items[0]);
	      }
      	}
      	panels[i].appendChild(hbox);
    }
  },	
  ...
  saveSettings: function() {
    ...
    try {
      ...
      this.changed = false;
    window.arguments[0].builder.rebuild();
    Dialog.moveToHbox();
    } catch(e) { util.dumpException(e);util.dumpException("dfsdfsdf"); }
  },
  ...
  updateMode: function (event) {
  	  ...
  	  var selectedIdx = event.target.getAttribute("idx");
  	  document.getElementById("modelist").setAttribute("selectedIdx", selectedIdx);
  	  window.arguments[0].setAttribute("mode", mode);
  },
}
